    ...
    try:
        ...
        for part in ["part1", "part2"]:
            ...
            try:
                ...
                for idx, (input_data, expected_output) in enumerate(
                    test_cases, start=1
                ):
                ...
                print(
                    f"    {bcolors.OKGREEN}✅ Final Output: Didn't Crash!{bcolors.ENDC}\n"
                )
            except AssertionError as e:
                print(f"    {bcolors.FAIL}❌ {e}{bcolors.ENDC}")
    except ModuleNotFoundError:
        print(
            f"{bcolors.WARNING}Day {day:02d} module not found. Skipping...{bcolors.ENDC}"
        )
    except AttributeError:
        print(
            f"{bcolors.WARNING}Day {day:02d} class not defined. Skipping...{bcolors.ENDC}"
        )
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        try:
            ...
            print(
                f"{bcolors.FAIL}Please provide a valid day number (e.g., `python test_runner.py 1`).{bcolors.ENDC}"
            )
    else:
